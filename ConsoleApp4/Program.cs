...
namespace ConsoleApp4
{
    internal class Program
    {
        ...
        static void GameDataSetting()
        {
            ...
            item0.isEquiped = false;

            Item item1;
            item1.Name = "낡은검";
            item1.Age = player.Atk + 3;
            item1.isEquiped = false;


            Item item2;
            item2.Name = "나무창";
            item2.Age = player.Atk + 5;
            item2.isEquiped = false;


            Item item3;
            item3.Name = "체력포션"; //소모품처리 하는법?
            item3.Age = player.Hp + 75;
            item3.isEquiped = false; // 게임을 저장기능을 만든 후 껏다 키면 계속 작동 될 가능성 있음.
        ...
        static void DisplayInveneq()
        {
            ...
            switch (input)
            {
                ...
                case 1:
                    Inventory[0].isEquiped = true;
                    Console.WriteLine("당신의 방어력이 증가했습니다.");
                    Thread.Sleep(1500);
                    DisplayInveneq();
                    break;

                case 2:
                    Inventory[1].isEquiped = true;
                    Console.WriteLine("당신의 공격력이 증가했습니다.");
                    Thread.Sleep(1500);
                    DisplayInveneq();
                    break;

                case 3:
                    Inventory[2].isEquiped = true;
                    Console.WriteLine("당신의 공격력이 많이 증가했습니다!");
                    Thread.Sleep(1500);
                    DisplayInveneq();
                    break;
